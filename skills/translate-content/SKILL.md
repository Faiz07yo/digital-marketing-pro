...
2. **Detect source language**: If the source language was not specified, run `python ${CLAUDE_PLUGIN_ROOT}/scripts/language-router.py --action detect --content "{content_or_path}"` to identify the source language with confidence score. Report the detected language to the user for confirmation if confidence is below 95%.
...
6. **Score translation quality**: Run `python ${CLAUDE_PLUGIN_ROOT}/scripts/language-router.py --action score --source "{source_content}" --translation "{translated_content}" --source-lang "{source}" --target-lang "{target}"` to assess quality across dimensions:
...
9. **Run brand voice check**: Execute `python ${CLAUDE_PLUGIN_ROOT}/scripts/brand-voice-scorer.py --brand {slug} --content "{translated_content}" --language "{target_lang}"` to assess whether the translated content maintains brand voice characteristics. Flag any voice drift with specific examples and suggestions.
